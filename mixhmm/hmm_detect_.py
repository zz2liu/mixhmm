"""hmm_detect.py - User interface with convenient wrappers.

11/09/08 created by Zongzhi Liu
11/19/08 mv here from hmm.py

todo: export (state, genotype, copy_number, allele_imbalance)
todo: deprecate this module use mixhmm.py instead.

"""
from __future__ import division
from lib25 import sys, groupby, itemgetter, warn, set_trace, sqlite
from numpy import log, array, sum, tile, diag, isnan, repeat, disp, roll
from scipy.stats import norm

from util import (TINY, DEFAULT_PFB, iter_regions, filter_regions)
from viterbi import  viterbi
from format import parse_hmm, parse_sample, parse_pfb
FM = 3 #normal state

###
# CNV detection interface
def detectCnv(self, pfb_file, sample_file, sel_chr=range(1, 23),
        sample_cols=[0,1,2,3,4], pfb_cols=[0, -1], min_snps=0):
    result = _detectCnv(self, pfb_file, sample_file,
            sel_chr=sel_chr, sample_cols=sample_cols, pfb_cols=pfb_cols)
    if min_snps:
        result = filter_regions(result, min_snps)
    return result

def _detectCnv(self, pfb_file, sample_file, bg_file=None, sel_chr=range(1, 23),
        sample_cols=range(5), pfb_cols=[0, -1], bg_cols=range(5)):
    ...
    bg_file: each line of (chr, start, end, p, bg)
